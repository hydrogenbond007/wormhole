...
func newMockGuardianSet(n int) []*mockGuardian {
	...
	for i := 0; i < n; i++ {
		...
		gs[i] = &mockGuardian{
			...
			gk:               gk,
			guardianAddr:     GuardianKeyToAddress(gk.PublicKey),
		}
	}
...
func testStatusServer(logger *zap.Logger, statusAddr string) {
	// Check /readyz
	for {
...
func TestNodes(t *testing.T) {
	...
	supervisor.New(rootCtx, zapLogger, func(ctx context.Context) error {
		...
		for i := range gs {
			testStatusServer(logger, fmt.Sprintf("http://127.0.0.1:%d", mockStatusPort(uint(i))))
		}
