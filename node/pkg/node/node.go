...
func GuardianOptionP2P(p2pKey libp2p_crypto.PrivKey, networkId string, bootstrapPeers string, nodeName string, disableHeartbeatVerify bool, port uint, ibcFeatures string) *GuardianOption {
	...
	return &GuardianOption{
		...
		f: func(ctx context.Context, logger *zap.Logger, g *G) error {
			...
			for _, w := range watcherConfigs {
				if _, ok := watchers[w.GetNetworkID()]; ok {
					logger.Fatal("NetworkID already configured", zap.String("network_id", string(w.GetNetworkID())))
				}

				watcherName := string(w.GetNetworkID()) + "watch"
				logger.Debug("Setting up watcher: " + watcherName)

				if w.GetNetworkID() != "solana-confirmed" { // TODO this should not be a special case, see comment in common/readiness.go
...
func (g *G) applyOptions(ctx context.Context, logger *zap.Logger, options []*GuardianOption) error {
	...
	for _, option := range options {
		...
		if _, ok := configuredComponents[option.name]; ok {
			return fmt.Errorf("Component %s has been configured already and cannot be configured a second time.", option.name)
		}
